...
class MainWindow(Qt.QMainWindow, frmMain.Ui_MainWindow):
    ...
    def __init__(self):
        ...
        for idx_vertical in range(len(self.modules) - 1):
            if type(self.modules[idx_vertical]) in (tuple, list):
                ...
                for idx_horizontal in range(len(self.modules[idx_vertical]) - 1):
                    self.modules[idx_vertical][idx_horizontal].add_receiver(
                        self.modules[idx_vertical][idx_horizontal + 1])
            else:
                # connect top/down
                if type(self.modules[idx_vertical + 1]) in (tuple, list):
        ...
        self.topmodule.connect(self, Qt.SIGNAL("parentevent(PyQt_PyObject)"), self.topmodule.parent_event,
    ...
    def processRemoteCommand(self, cmd_string):
        ...
        if len(cmd_string) > 0:
            ...
            if cmd not in ["1", "2", "3", "4", "M", "I", "S", "Q", "X", "F"]:
                ...
                error_message = u"some variables (1 Configuration file, 2 Experiment ID or 3 Subject ID) are not initialized: '%s'" % (
                    cmd_string)
    ...
    def writeBatteryLog(self, event):
        ...
        try:
            ...
            if not logdir.exists():
                if self.battery_log:
                    print(f"Battery Log is disabled because the log directory {logpath} doesn't exist")
                ...
                if self.battery_log:
                    print(f"Battery Log to {self.battery_logfile} is enabled")
        except Exception as e:
            print(e)
            self.battery_log = False
    ...
    def updateUtilization(self, utilization):
        ...
        if utilization < 80:
            self.progressBarUtilization.setStyleSheet(
                "QProgressBar {padding: 1px; text-align: right; margin-right: 35ex;} " \
...
def setpriority(priority=2):
    ...
    try:
        ...
        kernel32.GetProcessAffinityMask.argtypes = [wintypes.HANDLE, ctypes.POINTER(ctypes.c_int),
        ...
        print(f"INFO: Available CPUs (bit mask) {bin(smask)}, Python process limited to {bin(pmask)}")
        # print "INFO: Available CPUs (bit mask) 0x%04X, Python process limited to 0x%04X"%(smask, pmask)

    except Exception as e:
        tb = GetExceptionTraceBack()[0]
        print(f"INFO: the process priority can not be raised because PyWin32 is not installed or an error occurred.\n      - {tb}->{str(e)}")
